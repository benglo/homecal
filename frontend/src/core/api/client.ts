...
export const api = {
  ...
  setVoiceVolume: (level: number) =>
    send<{ ok: true; volume: number }>('PUT', '/api/voice/volume', { level }),
  setVoiceAudioMute: (muted: boolean) =>
    send<{ ok: true; audio_muted: boolean }>('PUT', '/api/voice/audio-mute', { muted }),
